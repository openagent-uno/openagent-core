...
from openagent_core.voice import voice as _voice
...
async def resolve_stt(db: Any) -> BaseSTT | None:
    ...
    if db is not None:
        ...
        if row is not None:
            ...
            if vendor == "deepgram":
                ...
                from openagent_core.voice.stt_deepgram import DeepgramStreamingSTT
    ...
    if runtime_environment().get("OPENAI_API_KEY"):

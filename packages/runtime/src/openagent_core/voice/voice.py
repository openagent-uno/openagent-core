...
async def transcribe(
    ...
    *,
    ...
    if not file_path:
        return None
    if not language:
        language = runtime_environment().get("OPENAGENT_VOICE_LANG") or None
...
async def _load_local_model():
    ...
    async with _WHISPER_LOCK:
        ...
        model_size = runtime_environment().get("OPENAGENT_WHISPER_MODEL", "small")
...
async def _transcribe_openai(file_path: str, *, language: str | None = None) -> str | None:
    api_key = runtime_environment().get("OPENAI_API_KEY")
    if not api_key:
        return None
...
def backend_available() -> bool:
    ...
    try:
        ...
        pass
    return bool(runtime_environment().get("OPENAI_API_KEY"))

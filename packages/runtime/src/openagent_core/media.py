...
class ToolStatusEvent:
    ...
    — see :mod:`openagent_core.mcp.servers.tool_search`), ``parse_status_event`` unwraps
    ...
    channel labels (:mod:`openagent_core.channels.tool_labels`) match the app's chips.
    """
    tool: str
...
def parse_compaction_status(raw: str) -> dict | None:
    ...
    ``openagent_core.core.compaction.compact`` fires its progress hints through the
